...
- 在图中用**显式视觉标记**区分：🆕 **新增**（粗框 / 高亮色）/ ✏️ **修改**（虚框 / 次要色）/ ⬜ **复用未改动**（默认色）。
- 若本次改造范围较大，可在图下追加一张 **变更前 → 变更后对比简图**（≤ 10 节点）。

```mermaid
flowchart LR
    Client[客户端] --> Gateway[API 网关]
    Gateway -->|HTTPS| OrderSvc[订单服务<br/>✏️修改]
    Gateway -->|HTTPS| PromoSvc[营销服务<br/>🆕新增]
    OrderSvc --> OrderDB[(MySQL: orders)]
    OrderSvc -.异步.-> MQ[[RocketMQ: order_topic]]
    PromoSvc --> PromoDB[(MySQL: promotions<br/>🆕新增)]
    MQ -.->|消费| PromoSvc
```
...
|---|---|---|---|---|---|
| 服务 | 订单服务 | ✏️ 修改 | 新增"营销叠加结算"分支 | `POST /v1/orders` 入参 `promoIds[]`，向后兼容 | FR-005 |
...
> 若清单超过 15 行，保留"新增 + 修改"两类全部条目，"复用未改动"在表中只列代表性 3–5 条 + 给出"完整复用清单见 `.tech-design/notes.md`"链接。
...
- [ ] 已给出"加上本需求后的整体架构图（合入视图）"，且用显式视觉标记区分🆕新增 / ✏️修改 / ⬜复用。
