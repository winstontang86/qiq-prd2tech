...
| Q-9a | 详细设计变更标注 | 已有系统演进时，§3 关键对象（存储 / 缓存 / MQ / DTO / 接口 / 流程含数据流图 / 状态机（如保留））均标注 `[新增]` / `[修改]` / `[复用]`，且与 §2.1 / §2.2 一致 | Blocker（已有系统演进时） |
...
| Q-31 | 全文长度与拆分 | 文档总行数默认 ≤ 1000 行；若超出，必须已按相对独立功能模块拆分，或在功能单一时按实现步骤 / 落地阶段拆分；仅人工明确确认放开时可单篇超过 1000 行，且元信息 `状态` 行有"篇幅放开：确认人 + 日期 + 理由"声明，`.qiqskills/backend-tech/notes.md` 记录确认依据 | Blocker |
...
| Q-34 | 用户反馈修订范围 | 若本轮为用户反馈修订，已在 `.qiqskills/backend-tech/notes.md` 记录 `FB-` 清单；正文改动仅覆盖用户反馈点、直接一致性影响和明确 Blocker；修改后已重跑 Step 4 / Step 5 / Step 7 | High |
| Q-35 | 超长方案拆分 | 最终方案超过 1000 行时，已拆分为多份方案（优先按相对独立功能模块；功能单一时按实现步骤 / 落地阶段），或已有人工明确确认放开；拆分后的每篇文档仍需独立满足 Q-1 至 Q-35 | Blocker |
...
| Q-31 | 全文长度与拆分 | ✅ | ≤ 1000 行 / 或已拆分 / 或已有人工确认放开记录 |
...
- **修复后必须重审**：任一 Blocker / High 整改后，需重新跑完 Q-1 至 Q-35 的检查并更新 `.qiqskills/backend-tech/quality-check.md`。
