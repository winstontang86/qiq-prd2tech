...
| Q-9 | P0 数据流图 | 每个 P0 FR 对应一张 Mermaid `sequenceDiagram`；不得用 `flowchart` / 流程图替代；需含核心数据项、存储 / 缓存 / MQ 读写点和异常 / 补偿分支 | High |
| Q-9a | 详细设计变更标注 | 已有系统演进时，§3 关键对象（存储 / 缓存 / MQ / DTO / 接口 / 流程 / 数据流图 / 状态机）均标注 `[新增]` / `[修改]` / `[复用]`，且与 §2.1 / §2.2 一致 | Blocker（已有系统演进时） |
...
| Q-13 | 状态机 | 涉及状态实体含流转表（含守卫与副作用）；状态图可选 | High |
...
| Q-25 | 反模式自检 | 08-anti-patterns 的反模式（A–Q）已逐条回答 | High |
...
14. **反馈修订引入无关重写**：用户仅要求局部修改时，最终方案发生无关章节重写、无依据新增目标或新技术选型，且未能说明与反馈点或 Blocker 的直接关系。
...
- **修复后必须重审**：任一 Blocker / High 整改后，需重新跑完 Q-1 至 Q-34 的检查并更新 `.qiqskills/backend-tech/quality-check.md`。
