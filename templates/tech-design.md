...
> 数据流图必须使用 Mermaid `sequenceDiagram`，不要使用 `flowchart` / 流程图；需表达参与方、调用顺序、核心数据项、存储 / 缓存 / MQ 读写点、同步 / 异步边界和异常 / 补偿回包。

#### 场景：{P0 业务场景 1}（[新增] / [修改] / [复用]，对应 FR-XXX）
...
> 说明：对应 FR / NFR、参与方职责、关键数据项、存储 / 缓存 / MQ 读写点、同步 / 异步边界、关键决策点、异常分支和补偿动作。
